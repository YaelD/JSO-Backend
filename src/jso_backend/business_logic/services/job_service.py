...
class JobService:
    ...
    async def update_job(self, id: int, job_details: dict[str, Any]) -> JobEntity:
        async with UnitOfWork().create() as unit_of_work:
            ...
            JobValidator().validate_and_update_job_data_fields(
                job_entity=job_to_update, job_details_to_update=job_details
            )
